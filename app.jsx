...
// ---- Sidebar
function Sidebar({ tab, setTab, savedCount }) {
  return (
    <aside className="nfg-sidebar">
      <div className="nfg-sidebar-brand">
        <div className="nfg-sidebar-by">by Soil to Soul</div>
        <div className="nfg-sidebar-ttl">Natural<br/>Food Guide</div>
      </div>
      <nav>
        <button className={'nfg-sidebar-btn' + (tab === 'guide' ? ' on' : '')} onClick={() => setTab('guide')}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M3 6l6-2 6 2 6-2v14l-6 2-6-2-6 2zM9 4v16M15 6v16"/>
          </svg>
          Guide
        </button>
        <button className={'nfg-sidebar-btn' + (tab === 'merkliste' ? ' on' : '')} onClick={() => setTab('merkliste')}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill={tab === 'merkliste' ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2">
            <path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/>
          </svg>
          Merkliste
          {savedCount > 0 ? <span className="nfg-sidebar-badge">{savedCount}</span> : null}
        </button>
        <button className={'nfg-sidebar-btn' + (tab === 'mehr' ? ' on' : '')} onClick={() => setTab('mehr')}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
// ---- Mobile bottom nav
function MobileNav({ tab, setTab, savedCount }) {
  return (
    <nav className="nfg-mobile-nav">
      ...
      <button className={tab === 'merkliste' ? 'on' : ''} onClick={() => setTab('merkliste')}>
        ...
        Merkliste
        {savedCount > 0 ? <span className="nfg-mobile-badge">{savedCount}</span> : null}
      </button>
  ...
  return (
    <header className="nfg-hd">
      <div className="nfg-hd-top">
        <div className="nfg-hd-ttl">Guide</div>
  ...
  return (
    <div className="nfg-cats">
      {CATEGORIES.map(c => {
        ...
        return (
          <button key={c.id} className={'nfg-cat' + (active ? ' on' : '')} onClick={() => toggle(c.id)}
...
function ListingCard({ item, onOpen, saved, onSave }) {
  return (
    <article className={'nfg-card' + (item.featured ? ' featured' : '')} onClick={() => onOpen(item)}>
      <div className="nfg-card-img">
        ...
        <button className={'nfg-save' + (saved ? ' on' : '')}
      ...
      <div className="nfg-card-body">
        ...
        <p className="nfg-card-short">{item.short}</p>
        ...
        <div className="nfg-profile-body">
          ...
          {item.top50 ? (
            <div className="nfg-top50-banner">
              ...
              <div><b>Top 50 Farmers</b><small>Ausgezeichnet 2026</small></div>
            </div>
          ) : null}

          <div className="nfg-actions">
            {item.book ? <button className="nfg-btn-primary">Book a Table</button> : null}
            <button className={'nfg-btn-ghost' + (liked ? ' on' : '')}
              ...
              {rel.sources.length > 0 ? (
                <div className="nfg-rel">
                  ...
                  <div className="nfg-rel-row">
                    {rel.sources.map(id => {
                      const r = findById(id); if (!r) return null;
                      return <button key={id} className="nfg-rel-chip" onClick={() => onOpen(r)}>
...
function GuidePage({ saved, onSave, onOpen }) {
  ...
  const filtered = useMemo(() => NFG_LISTINGS.filter(it => {
    ...
    <>
      ...
      {view === 'map' ? (
        <div className="nfg-map-section">
          ...
          })() : <div className="nfg-map-hint">Tipp: Pin auf der Karte anklicken für Kurzinfo.</div>}
        </div>
      ) : (
        <div className="nfg-content">
...
function MerklistePage({ saved, onSave, onOpen }) {
  ...
  return (
    <div className="nfg-page">
      ...
      <div className="nfg-list">
        {items.length === 0
          ? <div className="nfg-empty">Noch nichts gespeichert. Lesezeichen-Icon auf einer Karte anklicken.</div>
...
function MehrPage() {
  ...
  return (
    <div className="nfg-page">
      ...
      <div className="nfg-mehr-grid">
        <section className="nfg-more-card full">
          ...
          <div className="nfg-rubric">
            {[
              ...
              <div key={r.c} className="nfg-rubric-row">
                ...
                <div className="nfg-rubric-meta"><b>{r.c}</b><span>{r.w}%</span></div>
          ...
          <div className="nfg-mag">
            {[
              ...
              <article key={m.t} className="nfg-mag-card">
                ...
                <div><span className="nfg-mag-kind">{m.k}</span><h4>{m.t}</h4></div>
              </article>
            ))}
          </div>
        </section>
          ...
          <ul className="nfg-events">
            {events.map(e => (
              <li key={e.id}>
                ...
                <div><b>{e.name}</b><span>{e.city} · {e.short}</span></div>
...
function App() {
  ...
  return (
    <div className="nfg-shell">
      <Sidebar tab={tab} setTab={setTab} savedCount={saved.length}/>
      <main className="nfg-main">
      ...
      <MobileNav tab={tab} setTab={setTab} savedCount={saved.length}/>
      {open ? (
        <Profile item={open} onClose={() => setOpen(null)} saved={saved.includes(open.id)} onSave={onSave} onOpen={setOpen}/>
      ) : null}
    </div>
  );
}
